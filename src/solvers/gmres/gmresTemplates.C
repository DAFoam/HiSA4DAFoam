...
) const
{
    ...
    label nIter = dict.lookupOrDefault<label>("maxIter", 5);
    ...
    pInitRes.reset(new residualIO(tol));
    ...
    #if OPENFOAM >= 2506
    List<UPstream::Request> reduceRequest(nDirs);
    #else
    ...
    while(1)                                            // Outer loop
    {
        ...
        #if OPENFOAM >= 2506
        // Workaround
        reduce(&beta, 1, sumOp<scalar>(), Pstream::msgType(), UPstream::worldComm, reduceRequest[0]);
        #else
        ...
        for (label i = 0; i < nDirs; i++)				// Search directions
        {
            ...
            for (label j = 0; j <= i; j++)
            {
                ...
                #if OPENFOAM >= 2506
                // Workaround
                reduce(&(H[j][i]), 1, sumOp<scalar>(), Pstream::msgType(), UPstream::worldComm, reduceRequest[j]);
                #else
