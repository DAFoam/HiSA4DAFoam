...
#include "characteristicInletOutletTemperatureFvPatchScalarField.H"
...
Foam::characteristicInletOutletTemperatureFvPatchScalarField::
characteristicInletOutletTemperatureFvPatchScalarField
(
    const fvPatch& t,
...
characteristicInletOutletTemperatureFvPatchScalarField
(
    const characteristicInletOutletTemperatureFvPatchScalarField& ptf,
    const fvPatch& t,
    const DimensionedField<scalar, volMesh>& iF,
...
characteristicInletOutletTemperatureFvPatchScalarField
(
    const characteristicInletOutletTemperatureFvPatchScalarField& sfspvf
)
:
...
void Foam::characteristicInletOutletTemperatureFvPatchScalarField::updateCoeffs()
{
    if (!size() || updated())
    {
        return;
    ...
    forAll(pT, facei)
    {
        ...
        else if (pM[facei] <= 0.0)                   // Subsonic inflow
        {
            ...
            scalar prho = rhoInf[facei] - (pRef_ - pp[facei])/sqr(cO[facei]);
            pT[facei] = pp[facei]/(R[facei]*prho);
        }
        else                                         // Subsonic outflow
        {
            valueFraction()[facei] = 1 - pp[facei]/(R[facei]*(p[facei]/R[facei] - T[facei]*(p[facei]-pp[facei])/sqr(cO[facei])));
            pT[facei] = 0;
        }
    }
...
void Foam::characteristicInletOutletTemperatureFvPatchScalarField::write(Ostream& os) const
{
