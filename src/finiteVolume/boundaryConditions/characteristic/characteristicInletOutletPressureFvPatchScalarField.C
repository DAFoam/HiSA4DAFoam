...
#include "characteristicInletOutletPressureFvPatchScalarField.H"
...
Foam::characteristicInletOutletPressureFvPatchScalarField::
characteristicInletOutletPressureFvPatchScalarField
(
    const fvPatch& p,
...
characteristicInletOutletPressureFvPatchScalarField
(
    const characteristicInletOutletPressureFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
characteristicInletOutletPressureFvPatchScalarField
(
    const characteristicInletOutletPressureFvPatchScalarField& sfspvf
)
:
...
void Foam::characteristicInletOutletPressureFvPatchScalarField::updateCoeffs()
{
    if (!size() || updated())
    {
        return;
    ...
    const scalarField cO(sqrt(pgamma.patchInternalField()/ppsi.patchInternalField()));
    ...
    forAll(pp, facei)
    {
        ...
        else if (pM[facei] <= 0.0)                   // Subsonic inflow
        {
            ...
            vector Ub = URef_;
            refGrad()[facei] = 
                ((rhoO[facei]*cO[facei]) * ((U[facei]-Ub) & pn[facei]))*deltaCoeffs[facei];
        }
...
void Foam::characteristicInletOutletPressureFvPatchScalarField::write(Ostream& os) const
{
