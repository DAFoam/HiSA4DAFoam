...
void Foam::characteristicFarfieldPressureFvPatchScalarField::updateCoeffs()
{
    ...
    forAll(pp, facei)
    {
        ...
        else if (pM[facei] <= 0.0)                   // Subsonic inflow
        {
            ...
            pp[facei] = pRef_ - (rhoO[facei]*cO[facei]) * ((URef_ - U[facei]) & pn[facei]);
        }
        else                                         // Subsonic outflow
        {
            // Lohner - prescribed p and u_n
            // Provides better reflection properties when compared to 
            // prescribed p (see testBc case)
            valueFraction()[facei] = 0.5;
            pp[facei] = pRef_ - (rhoO[facei]*cO[facei]) * ((URef_ - U[facei]) & pn[facei]);
        }
