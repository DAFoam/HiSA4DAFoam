...
void Foam::characteristicFarfieldVelocityFvPatchVectorField::updateCoeffs()
{
    ...
    forAll(Up, facei)
    {
        ...
        else if (pM[facei] <= 0.0)                   // Subsonic inflow
        {
            ...
            // Prescribed u
            Up[facei] = URef_ - pn[facei]*(pRef_ - pp[facei])/(rhoO[facei]*cO[facei]);
        }
        else                                         // Subsonic outflow
        {
            // Prescribed p and u_n - Lohner (modified)
            // Extrapolated u_t
            Up[facei] = (URef_&pn[facei])*pn[facei] - pn[facei]*(pRef_ - p[facei])/(rhoO[facei]*cO[facei]) + (U[facei]-(U[facei]&pn[facei])*pn[facei]);
        }
    }
