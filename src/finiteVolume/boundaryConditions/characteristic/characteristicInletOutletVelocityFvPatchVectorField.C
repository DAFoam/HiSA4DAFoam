...
#include "characteristicInletOutletVelocityFvPatchVectorField.H"
...
Foam::characteristicInletOutletVelocityFvPatchVectorField::
characteristicInletOutletVelocityFvPatchVectorField
(
    const fvPatch& p,
...
characteristicInletOutletVelocityFvPatchVectorField
(
    const characteristicInletOutletVelocityFvPatchVectorField& ptf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
characteristicInletOutletVelocityFvPatchVectorField
(
    const characteristicInletOutletVelocityFvPatchVectorField& sfspvf
)
:
...
void Foam::characteristicInletOutletVelocityFvPatchVectorField::updateCoeffs()
{
    if (!size() || updated())
    {
        return;
    ...
    forAll(Up, facei)
    {
        ...
        else                                         // Subsonic outflow
        {
            ...
            valueFraction()[facei] = 0;
            refGrad()[facei] = 
                pn[facei]*(p[facei] - pRef_)/(rhoO[facei]*cO[facei])*deltaCoeffs[facei];
        }
...
void Foam::characteristicInletOutletVelocityFvPatchVectorField::write(Ostream& os) const
{
